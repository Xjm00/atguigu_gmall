...
public class GwareServiceImpl implements GwareService {
    ...
    public void sendToOrder(WareOrderTask wareOrderTask) throws JMSException{
            ...
            mapMessage.setString("trackingNo",wareOrderTask.getTrackingNo());   // 追踪号
...
//                updateStatusWareOrderTaskByOrderId(wareOrderTask.getOrderId(),TaskStatus.SPLIT);
    ...
    /***支付已成功，订单状态为“待付款状态”，判断库存是否有库存情况***************************************************/
    @Override
    public void checkGwareStatus(String out_trade_no){

    }

    /***
     * 库存锁定成功，准备出库，发送消息，由订单系统消费，修改状 态为商品准备出库
     * @param wareOrderTask
     * @throws JMSException
     */
